...
| `CLAUDE_MODEL` | 任意 | AI生成に使うモデルの上書き(デフォルト: `claude-sonnet-5`) | — |
| `CLAUDE_EFFORT` | 任意 | 思考の深さの上書き(デフォルト: `xhigh`) | — |

- キーなしでも、フラッシュカード・記事リーダー・ダッシュボードは全部使えます
- **NewsAPI 無料枠の注意**: 開発用途のみ / 過去1ヶ月の記事 / 1日100リクエスト程度 / 記事の出典表示が必要(アプリ内で出典・元記事リンクを表示済み)
- **Anthropic APIキーがない場合**も問題生成は動きます(無料の簡易ロジック: 頻度ベースの語彙抽出+キーワード穴埋め問題)。キーを設定すると **Claude Sonnet 5**(アダプティブ思考 + `effort: xhigh` + 構造化出力)による読解4択クイズ+意味つき語彙抽出に自動で切り替わります。この設定はSonnet 5から最大限の品質を引き出す構成で、コストを抑えたい場合は `CLAUDE_EFFORT=medium` に下げてください
